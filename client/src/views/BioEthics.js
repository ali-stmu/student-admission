...
const BioEthics = () => {
      ...
      try {
        ...
        if (studentInfoFromStorage) {
          ...
          const response = await fetch(
            `${BASE_URL}bioethics-form/${studentInfo.user_id}`
          );
          if (response.ok) {
            ...
            console.error("Failed to fetch bioethics form data.");
          }
        }
      } catch (error) {
        console.error("Error fetching bioethics form data:", error);
      }
    };
  ...
  const handleChallanDownload = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}download-challan-bioethics/${formData.user_id}`
      );
      if (response.ok) {
  ...
  const handleSaveChallan = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}upload-challan-bioethics`, {
        method: "POST",
        body: formDataToSend,
      });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}savebioethicsform`, {
        method: "POST",
        body: formDataToSend,
      });
